...
mat4 drawOLPZoom(OneLayerPlanetProgram &planet, mat4 origin, mat4 NormalMatrix, mat4 ProjMatrix, float selfRotation, float angle, GLuint texture, Sphere sphere)
{
    ...
    // mat4 MVMatrix = rotate(origin, 1.f, vec3(0.f, 0.f, angle));     // Obliquity to Orbit
...
int main(int argc, char** argv) {
    ...
    float quadVertices[] = {
        ...
        -.75f,  .75f, 0.0f,  0.0f, 1.0f,
        -.75f, -.75f, 0.0f,  0.0f, 0.0f,
         .75f, -.75f, 0.0f,  1.0f, 0.0f,

        -.75f,  .75f, 0.0f,  0.0f, 1.0f,
         .75f, -.75f, 0.0f,  1.0f, 0.0f,
         .75f,  .75f, 0.0f,  1.0f, 1.0f
    };
    ...
    glVertexAttribPointer(VERTEX_ATTR_POSITION, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);
    glVertexAttribPointer(VERTEX_ATTR_COORD, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)(3 * sizeof(float)));
    ...
    auto imageData = loadImage("../assets/textures/Space/Star.jpg");
    if (imageData) {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, imageData->getWidth(), imageData->getHeight(), 0, GL_RGBA, GL_FLOAT, imageData->getPixels());
        std::cout << "Loading Space/Star texture" << std::endl;
    } else {
        std::cout << "Failed to load Space/Star texture" << std::endl;
    }
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    
    glBindTexture(GL_TEXTURE_2D, 0);
    Program background = loadProgram(  applicationPath.dirPath() + "shaders/background.vs.glsl",
                                        applicationPath.dirPath() + "shaders/background.fs.glsl");
    /*********************************
    ...
    while(!done) {
        // Init matrix
        mat4 ProjMatrix = perspective(radians(70.f), width/height, 0.1f, 100.f);
        ...
        if (!pause)
        {
            ...
            background.use();
            glUniform1i(glGetUniformLocation(background.getGLId(), "uTexture"), 0);
            ...
            ***/
            ...
            auto time = (windowManager.getTime()/20) * speed;
            if (all)
            {
                mat4 sunMVMatrix = drawOLPZoom(sunProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * 14.4f, 1.f, textures[0], sphere);

                drawTLP(mercuryProgram, sunMVMatrix, NormalMatrix, ProjMatrix, time * 58.65f, time * 4.09f,
                                    -2.9f, 9.f, 0.206f, 0.2f, 0.2f, 0.2f, textures[1], textures[2], sphere);

                drawTLP(venusProgram, sunMVMatrix, NormalMatrix, ProjMatrix, time * -243.02f, time * 1.60f,
                                    -5.4f, 3.4f, 0.007f, 0.4f, 0.4f, 0.4f, textures[3], textures[4], sphere);
                
                mat4 earthMVMatrix = drawTLP(earthProgram, sunMVMatrix, NormalMatrix, ProjMatrix, time, time * 0.98f,
                                    -7.5f, 1.f, 0.017f, 0.5f, 0.5f, 0.5f, textures[5], textures[6], sphere);
                drawOLP(moonProgram, earthMVMatrix, NormalMatrix, ProjMatrix, time * 27.32f, time * 13.1f,
                                    -2.f, 1.f, 0.055f, 0.2f, 0.2f, 0.2f, textures[7], sphere);

                mat4 marsMVMatrix = drawOLP(marsProgram, sunMVMatrix, NormalMatrix, ProjMatrix, time * 1.02f, time * 0.52f,
                ...
                mat4 jupiterMVMatrix = drawOLP(jupiterProgram, sunMVMatrix, NormalMatrix, ProjMatrix, time * 0.41f, time * 0.08f,
                                    -38.9f, 1.3f, 0.049f, 0.9f, 0.9f, 0.9f, textures[11], sphere);                                    
                drawOLP(callistoProgram, jupiterMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    18.82f, 0.19f, 0.007f, 0.2410f, 0.2410f, 0.2410f, textures[10], sphere);    
                drawOLP(ganymedeProgram, jupiterMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    10.70f, 0.18f, 0.001f, 0.2631f, 0.2631f, 0.2631f, textures[10], sphere);    
                drawOLP(europaProgram, jupiterMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    6.71f, 0.47f, 0.009f, 0.1560f, 0.1560f, 0.1560f, textures[10], sphere);    
                drawOLP(ioProgram, jupiterMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    4.21f, 0.04f, 0.004f, 0.1821f, 0.1821f, 0.1821f, textures[10], sphere);  

                mat4 saturnMVMatrix = drawOLP(saturnProgram, sunMVMatrix, NormalMatrix, ProjMatrix, time * 0.45f, time * 0.03f,
                                    -71.6f, 2.5f, 0.052f, 0.8f, 0.8f, 0.8f, textures[12], sphere); 
                drawOLP(mimasProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    1.86f, 1.53f, 0.0202f, 0.0208f, 0.0197f, 0.0191f, textures[10], sphere);    
                drawOLP(enceladusProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    2.38f, 0.f, 0.0045f, 0.0257f, 0.0251f, 0.0248f, textures[10], sphere);    
                drawOLP(tethysProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    2.95f, 1.86f, 0.f, 0.0538f, 0.0528f, 0.0526f, textures[10], sphere);    
                drawOLP(dioneProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    3.77f, 0.02f, 0.0022f, 0.0563f, 0.0561f, 0.0560f, textures[10], sphere);    
                drawOLP(rheaProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    5.27f, 0.35f, 0.0010f, 0.0765f, 0.0763f, 0.0762f, textures[10], sphere);    
                drawOLP(titanProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    12.22f, 0.33f, 0.0292f, 0.2575f, 0.2575f, 0.2575f, textures[10], sphere);    
                drawOLP(hyperionProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    15.01f, 0.43f, 0.1042f, 0.0180f, 0.0133f, 0.0103f, textures[10], sphere);    
                drawOLP(iapetusProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    35.61f, 14.72f, 0.0283f, 0.0746f, 0.0746f, 0.0712f, textures[10], sphere);  

                mat4 uranusMVMatrix = drawOLP(uranusProgram, sunMVMatrix, NormalMatrix, ProjMatrix, time * -0.72f, time * 0.01f,
                                    -143.4f, 0.8f, 0.047f, 0.7f, 0.7f, 0.7f, textures[14], sphere);  
                drawOLP(mirandaProgram, uranusMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        1.3f, 4.34f, 0.0013f, 0.0240f, 0.0234f, 0.0233f, textures[10], sphere);
                drawOLP(arielProgram, uranusMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    1.91f, 0.04f, 0.0012f, 0.0581f, 0.0578f, 0.0578f, textures[10], sphere);    
                drawOLP(umbrielProgram, uranusMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    2.66f, 0.13f, 0.0039f, 0.0585f, 0.0585f, 0.0585f, textures[10], sphere);    
                drawOLP(titaniaProgram, uranusMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    4.36f, 0.08f, 0.0011f, 0.0789f, 0.0789f, 0.0789f, textures[10], sphere);    
                drawOLP(oberonProgram, uranusMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    5.84f, 0.07f, 0.0014f, 0.0761f, 0.0761f, 0.0761f, textures[10], sphere);  

                mat4 neptuneMVMatrix = drawOLP(neptuneProgram, sunMVMatrix, NormalMatrix, ProjMatrix, time * 0.67f, time * 0.006f,
                                    -225.8f, 1.8f, 0.010f, 0.6f, 0.6f, 0.6f, textures[14], sphere);                                      
                drawOLP(tritonProgram, neptuneMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    3.55f, 157.345f, 0.000016f, 0.1353f, 0.1353f, 0.1353f, textures[10], sphere);
                drawOLP(nereidProgram, neptuneMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                    55.13f, 7.23f, 0.7512f, 0.0170f, 0.0170f, 0.0170f, textures[10], sphere);

                mat4 plutoMVMatrix = drawTLP(plutoProgram, sunMVMatrix, NormalMatrix, ProjMatrix, time * -6.39f, time * 0.004f,
                                    -295.3f, 17.2f, 0.244f, 0.1f, 0.1f, 0.1f, textures[17], textures[18], sphere);
                drawOLP(charonProgram, plutoMVMatrix, NormalMatrix, ProjMatrix, time, time,
                    1.9596f, 0.00005f, 0.f, 0.0606f, 0.0606f, 0.0606f, textures[10], sphere);
            } else
            {
                if (planets.at(0))
                {
                    drawOLPZoom(sunProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * 14.4f, 1.f, textures[0], sphere);
                }
                if (planets.at(1))
                {
                    drawTLPZoom(mercuryProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * 58.65f, 0.034f, textures[1], textures[2], sphere);
                }
                if (planets.at(2))
                {
                    drawTLPZoom(venusProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * -243.02f, 177.4f, textures[3], textures[4], sphere);
                }
                if (planets.at(3))
                {
                    mat4 earthMVMatrix = drawTLPZoom(earthProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time, 23.4f, textures[5], textures[6], sphere);
                    drawOLP(moonProgram, earthMVMatrix, NormalMatrix, ProjMatrix, time * 27.32f, time * 13.2f,
                                        3.84f, 1.f, 0.055f, 0.35f, 0.35f, 0.35f, textures[7], sphere);
                }
                if (planets.at(4))
                {
                    mat4 marsMVMatrix = drawOLPZoom(marsProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * 1.02f, 25.2f, textures[8], sphere);
                    // drawOLP(deimosProgram, marsMVMatrix, NormalMatrix, ProjMatrix, time, time,
                ...
                if (planets.at(5))
                {
                    mat4 jupiterMVMatrix = drawOLPZoom(jupiterProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * 0.41f, 3.1f, textures[11], sphere);
                    drawOLP(callistoProgram, jupiterMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        18.82f, 0.19f, 0.007f, 0.2410f, 0.2410f, 0.2410f, textures[10], sphere);    
                    drawOLP(ganymedeProgram, jupiterMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        10.70f, 0.18f, 0.001f, 0.2631f, 0.2631f, 0.2631f, textures[10], sphere);    
                ...
                if (planets.at(6))
                {
                    mat4 saturnMVMatrix = drawOLPZoom(saturnProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * 0.45f, 26.7f, textures[12], sphere);
                    drawOLP(mimasProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        1.86f, 1.53f, 0.0202f, 0.0208f, 0.0197f, 0.0191f, textures[10], sphere);    
                    drawOLP(enceladusProgram, saturnMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        2.38f, 0.f, 0.0045f, 0.0257f, 0.0251f, 0.0248f, textures[10], sphere);    
                ...
                if (planets.at(7))
                {
                    mat4 uranusMVMatrix = drawOLPZoom(uranusProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * -0.72f, 97.8f, textures[14], sphere);
                    drawOLP(mirandaProgram, uranusMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        1.3f, 4.34f, 0.0013f, 0.0240f, 0.0234f, 0.0233f, textures[10], sphere);
                    drawOLP(arielProgram, uranusMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        1.91f, 0.04f, 0.0012f, 0.0581f, 0.0578f, 0.0578f, textures[10], sphere);    
                ...
                if (planets.at(8))
                {
                    mat4 neptuneMVMatrix = drawOLPZoom(neptuneProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * 0.67f, 17.2f, textures[16], sphere);
                    drawOLP(tritonProgram, neptuneMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        3.55f, 157.345f, 0.000016f, 0.1353f, 0.1353f, 0.1353f, textures[10], sphere);
                    drawOLP(nereidProgram, neptuneMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        55.13f, 7.23f, 0.7512f, 0.0170f, 0.0170f, 0.0170f, textures[10], sphere);
                ...
                if (planets.at(9))
                {
                    mat4 plutoMVMatrix = drawTLPZoom(plutoProgram, globalMVMatrix, NormalMatrix, ProjMatrix, time * -6.39f, 122.5f, textures[17], textures[18], sphere);
                    drawOLP(charonProgram, plutoMVMatrix, NormalMatrix, ProjMatrix, time, time,
                                        1.9596f, 0.00005f, 0.f, 0.0606f, 0.0606f, 0.0606f, textures[10], sphere);
                }
            }
