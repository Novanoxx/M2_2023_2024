...
extern bool g2x_PlugCapture(const char *basename, int dlx, int dly, int urx, int ury)
{
  ...
  { fprintf(stderr,"\e[1;31m<g2x_PlugCapture> : Erreur Creation fichier temporaire <%s>\e[0m\n",tempsnapname); return false;}
  return true;
}
...
extern bool g2x_Snapshot(const char *format, const char *basename, int w, int h)
{
  ...
  /* les tailles des snapshots sont alignés %8 (pour compatibilité formats JPEG/video) */
  ...
  if (match_string(format,pnm_name))
    ...
    snprintf(command,1023,"cat %s | pnmflip -tb | pnmtops -noturn -rle -dpi 600 > %s",tempsnapname,snapname);
  else
  {
...
extern bool g2x_FilmFrame(void)
{
  ...
  {
    fprintf(stderr,"\e[1;31mNombre maximal d'images (%d) atteint. Arret automatique\e[0m\n"
                  ,_MAXIMAGES_);
    return false;
  }
  else if (framenum==1)
...
mencoder -quiet mf://%s/*.jpg -mf fps=%d:type=jpg -oac copy -o %s.avi -ovc x264 -x264encopts \
bitrate=%d:autoaspect=1:subq=6:partitions=all:me=umh:frameref=5:bframes=3:b_pyramid:weight_b:threads=auto:pass=1"
                       ,tempdirname,fps,capturename,bitrate);
  _r_=system(command);
  snprintf(command,2047,"\
mencoder -quiet mf://%s/*.jpg -mf fps=%d:type=jpg -oac copy -o %s.x264.avi -ovc x264 -x264encopts \
bitrate=%d:autoaspect=1:subq=6:partitions=all:me=umh:frameref=5:bframes=3:b_pyramid:weight_b:threads=auto:pass=2"
                       ,tempdirname,fps,capturename,bitrate);
  _r_=system(command);
  return true;
}
...
{
  snprintf(command,2047,"\
mencoder -quiet mf://%s/*.jpg -mf fps=%d:type=jpg -o %s.mp4 -ovc lavc -lavcopts \
...
precmp=2:cmp=2:subcmp=2:preme=2:o=quantizer_noise_shaping=2:threads=12"
                       ,tempdirname,fps,capturename,bitrate);
  _r_=system(command);
  return true;
}
...
{
  snprintf(command,2047,"\
mencoder mf://%s/*.jpg -mf fps=%d:type=jpg -of lavf -ovc lavc -nosound\
-lavcopts vcodec=flv:autoaspect=1:vbitrate=%d:mbd=2:o=mpv_flags=+mv0:\
trell:v4mv:o=mpv_flags=+cbp_rd:last_pred=3 -o %s.flv"
