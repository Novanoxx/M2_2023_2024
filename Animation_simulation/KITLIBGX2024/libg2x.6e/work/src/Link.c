...
static void update_Hook(Link *L)
{
    ...
    double u = Vecteur(L->M1->pos,L->M2->pos)/d; // direction M1 → M2
    double F = -L->k*(d-L->l)*u; // force de rappel
...
{
    double F = -L->z*(L->M2->vit-L->M1->vit); // force de freinage
    L->M1->frc += F; // distribution sur M1
    L->M2->frc -= F; // distribution sur M2
}
...
static void update_Damped_Hook(Link *L)
{
    float d = distance(L->M1->pos,L->M2->pos); // distance courante ∣∣−−−→M1M2∣∣
    double u = Vecteur(L->M1->pos,L->M2->pos)/d; // direction M1 → M2
    double F = -L->k*(d-L->l)*u -L->z*(L->M2->vit-L->M1->vit); // force combinées
    L->M1->frc += F; // distribution sur M1
    L->M2->frc -= F; // distribution sur M2
}
