...
public:
    ...
    bool _showAxis = true;      // true to display the axis
    bool _showGrid = false;     // true to display a grid 
    bool _showValue = false;    // true to display the value of each object near the object
    bool _orientedLine = false; // true to add an arrow on a line to show its direction
    std::vector<float> _backgroundColor = {1.,1.,1.};  // color of the background (RGB, from 0 to 255 per component)
...
public:
    ...
    inline void showAxis(const bool showAxisOn) {_showAxis = showAxisOn;}


    /// \bief setter to enable / disable the option to show a grid
    inline void showGrid(const bool showGridOn) {_showGrid = showGridOn;}


    /// \bief setter to enable / disable the option to show the value of each object 
    inline void showValue(const bool showValueOn) {_showValue = showValueOn;}


    /// \bief setter to enable / disable the option to show an arrow that specifies the line orientation
    inline void setOrientedLine(const bool orientedLineOn) {_orientedLine = orientedLineOn;}


    /// \brief setter to choose the color of the background (default is white)
    inline void setBackgroundColor(const unsigned char R, const unsigned char G, const unsigned char B){
        _backgroundColor = {R/255.f,G/255.f,B/255.f};
    }
